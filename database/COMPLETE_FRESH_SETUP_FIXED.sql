...
CREATE OR REPLACE FUNCTION get_case_timeline(p_case_id VARCHAR(255)) RETURNS TABLE (id INT, event_type VARCHAR, event_title VARCHAR, event_description TEXT, status VARCHAR, role VARCHAR, username VARCHAR, timestamp_val TIMESTAMP) AS $$
BEGIN
    RETURN QUERY SELECT ct.id::INT, ct.event_type, ct.event_title, ct.event_description, ct.status, ct.role, u.username, ct.timestamp FROM case_timeline ct JOIN users u ON ct.user_id = u.id WHERE ct.case_id = p_case_id ORDER BY ct.timestamp DESC;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    INSERT INTO case_timeline (case_id, user_id, role, event_type, event_title, event_description, status) VALUES (p_case_id, p_changed_by, p_changed_by_role, 'status_change', 'Status Changed', p_reason, p_new_status);
...
BEGIN
    ...
    INSERT INTO case_timeline (case_id, user_id, role, event_type, event_title, event_description, status) VALUES (p_case_id, p_officer_id, 'Officer', 'investigated', 'Investigation', p_findings, 'ongoing');
