...
public class OfficerCaseDetailActivity extends AppCompatActivity {
    ...
    private MaterialButton btnSendSmsBottom;  // ✅ SMS button at bottom
    ...
    private boolean resolvedToastShown = false;  // ✅ Prevent duplicate resolved toast
    ...
    private void setupRecyclerViews() {
        ...
        caseProgressAdapter = new InvestigationStepAdapter(caseProgressSteps, new InvestigationStepAdapter.OnStepActionListener() {
            ...
            @Override
            public void onSendSms(InvestigationStep step) {
                // Case Progress is read-only, no SMS
            }
        });
        ...
        investigationActionsAdapter = new InvestigationStepAdapter(investigationActionSteps, new InvestigationStepAdapter.OnStepActionListener() {
            ...
            public void onStepAction(InvestigationStep step) {
                ...
                // ✅ Mark step as IN PROGRESS (show hourglass icon)
            ...
            @Override
            public void onSendSms(InvestigationStep step) {
                // ✅ Open SMS dialog for Officer to send notification
                openSmsDialog(step);
            }
        ...
        // ✅ Show numbers for Investigation Actions only
        investigationActionsAdapter.setShowNumbers(true);
        
        // ✅ Disable all buttons if case is resolved
        boolean isCaseResolved = currentReport != null && 
            currentReport.getStatus() != null && 
            currentReport.getStatus().equalsIgnoreCase("RESOLVED");
        investigationActionsAdapter.setCaseResolved(isCaseResolved);
        
        // ✅ Setup SMS button at bottom
        btnSendSmsBottom = findViewById(R.id.btnSendSmsBottom);
        if (btnSendSmsBottom != null) {
            btnSendSmsBottom.setOnClickListener(v -> {
                // Open SMS dialog
                if (currentReport != null) {
                    InvestigationStep dummyStep = new InvestigationStep("SMS", "Send SMS", "Send notification", "sms");
                    openSmsDialog(dummyStep);
                }
            });
        }
    ...
    private void initializeInvestigationTimeline() {
        ...
        // Action 1: Record Witness (ENABLED only if investigation started)
        ...
        actionWitness.setEnabled(isInvestigationStarted); // ✅ Enabled only if investigation started
    ...
    private void updateButtonVisibility() {
        ...
        // Hide all action buttons when resolved
            ...
            // Show a message that the case is resolved (only once)
            if (!resolvedToastShown) {
                Toast.makeText(this, "This case has been resolved and is now closed.", Toast.LENGTH_SHORT).show();
                resolvedToastShown = true;
            }
        }
    ...
    private void openAddWitness() {
        // Mark step as "In Progress" with hourglass icon
        markInvestigationStepInProgress("Record Witness");
        
        AddWitnessDialogFragment dialog = AddWitnessDialogFragment.newInstance(reportId, witness -> {
            // Witness added successfully - Mark as completed and refresh UI
            markInvestigationStepCompleted("Record Witness");
            loadCaseDetails();
        });
    ...
    private void openAddSuspect() {
        // Mark step as "In Progress" with hourglass icon
        markInvestigationStepInProgress("Identify");
        
        AddSuspectDialogFragment dialog = AddSuspectDialogFragment.newInstance(reportId, suspect -> {
            // Suspect added successfully - Mark as completed and refresh UI
            markInvestigationStepCompleted("Identify");
            loadCaseDetails();
        });
    ...
    private void openAddEvidence() {
        // Mark step as "In Progress" with hourglass icon
        markInvestigationStepInProgress("Gather Evidence");
        
        AddEvidenceDialogFragment dialog = AddEvidenceDialogFragment.newInstance(reportId, evidence -> {
            // Evidence added successfully - Mark as completed and refresh UI
            markInvestigationStepCompleted("Gather Evidence");
            loadCaseDetails();
        });
    ...
    private void openCreateHearing() {
        // Mark step as "In Progress" with hourglass icon
        markInvestigationStepInProgress("Schedule Hearings");
        
        ScheduleHearingDialogFragment dialog = ScheduleHearingDialogFragment.newInstance(reportId, hearing -> {
            // Hearing scheduled successfully - Mark as completed and refresh UI
            markInvestigationStepCompleted("Schedule Hearings");
            loadCaseDetails();
        });
    ...
    private void openDocumentResolution() {
        // Mark step as "In Progress" with hourglass icon
        markInvestigationStepInProgress("Document Resolution");
        
        DocumentResolutionDialogFragment dialog = DocumentResolutionDialogFragment.newInstance(reportId, resolution -> {
            // Resolution documented successfully - Mark as completed and update case status
            markInvestigationStepCompleted("Document Resolution");
            updateCaseStatusToResolved();
        });
        dialog.show(getSupportFragmentManager(), "DocumentResolution");
    }
    
    private void openSmsDialog(InvestigationStep step) {
        // ✅ Open SMS dialog for Officer to send SMS notification
        if (currentReport == null) return;
        
        String caseNumber = currentReport.getCaseNumber();
        String respondentName = currentReport.getRespondentName();
        String hearingDate = "TBD";  // You can get this from hearings if available
        String hearingTime = "TBD";
        String resolutionType = "Pending";
        ...
        OfficerSendSmsDialogFragment dialog = OfficerSendSmsDialogFragment.newInstance(
            ...
            new OfficerSendSmsDialogFragment.OnSmsSentListener() {
                ...
                public void onSmsSent(String phoneNumber, String messageType) {
                    android.util.Log.d("OfficerCaseDetail", "✅ SMS sent: " + messageType + " to " + phoneNumber);
                }
                
                @Override
                public void onSmsFailed(String errorMessage) {
                    android.util.Log.e("OfficerCaseDetail", "❌ SMS failed: " + errorMessage);
                }
            }
        );
    ...
    private void updateCaseStatusToResolved() {
        Executors.newSingleThreadExecutor().execute(() -> {
            try {
                if (currentReport != null && database != null) {
                    // ✅ Update case status to RESOLVED
                    currentReport.setStatus("RESOLVED");
                    database.blotterReportDao().updateReport(currentReport);
                    android.util.Log.d("OfficerCaseDetail", "✅ Case status updated to RESOLVED in database");
                    
                    // ✅ Sync to API for all 3 roles (Officer, Admin, User)
                    NetworkMonitor networkMonitor = new NetworkMonitor(this);
                    if (networkMonitor.isNetworkAvailable()) {
                        ApiClient.getApiService().updateReport(reportId, currentReport).enqueue(new Callback<BlotterReport>() {
                            @Override
                            public void onResponse(Call<BlotterReport> call, Response<BlotterReport> response) {
                                if (response.isSuccessful()) {
                                    android.util.Log.d("OfficerCaseDetail", "✅ Case status synced to API");
                                }
                            }
                            
                            @Override
                            public void onFailure(Call<BlotterReport> call, Throwable t) {
                                android.util.Log.w("OfficerCaseDetail", "⚠️ API sync failed: " + t.getMessage());
                            }
                        });
                    }
                    ...
                    runOnUiThread(() -> {
                        loadCaseDetails();
                        Toast.makeText(OfficerCaseDetailActivity.this, "✅ Case marked as Resolved", Toast.LENGTH_SHORT).show();
                    });
                }
            } catch (Exception e) {
                android.util.Log.e("OfficerCaseDetail", "Error updating case status: " + e.getMessage());
                runOnUiThread(() -> {
                    Toast.makeText(OfficerCaseDetailActivity.this, "Error updating case status", Toast.LENGTH_SHORT).show();
                });
            }
        });
    }
    
    private void markInvestigationStepInProgress(String stepTitle) {
        // ✅ Update UI immediately - show hourglass icon
        for (InvestigationStep step : investigationActionSteps) {
            if (step.getTitle().equals(stepTitle)) {
                step.setInProgress(true);
                step.setCompleted(false);
                android.util.Log.d("OfficerCaseDetail", "⏳ Marking step as In Progress: " + stepTitle);
                break;
            }
        }
        // Refresh the adapter to show hourglass icon
        runOnUiThread(() -> {
            if (investigationActionsAdapter != null) {
                investigationActionsAdapter.notifyDataSetChanged();
            }
    ...
    private void markInvestigationStepCompleted(String stepTitle) {
        // ✅ Update UI immediately - show green checkmark icon
        for (InvestigationStep step : investigationActionSteps) {
            if (step.getTitle().equals(stepTitle)) {
                step.setCompleted(true);
                step.setInProgress(false);
                android.util.Log.d("OfficerCaseDetail", "✅ Marking step as Completed: " + stepTitle);
                break;
            }
        }
        // Refresh the adapter to show checkmark icon
        runOnUiThread(() -> {
            if (investigationActionsAdapter != null) {
                investigationActionsAdapter.notifyDataSetChanged();
            }
