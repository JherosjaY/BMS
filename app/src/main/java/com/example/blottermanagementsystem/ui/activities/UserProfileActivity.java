...
public class UserProfileActivity extends BaseActivity {
        ...
        imagePickerLauncher = registerForActivityResult(
            ...
            uri -> {
                if (uri != null) {
                    ...
                    android.util.Log.d("UserProfileActivity", "=== PROFILE PICTURE SELECTED ===");
                    android.util.Log.d("UserProfileActivity", "Selected URI: " + selectedImageUri);
                    android.util.Log.d("UserProfileActivity", "currentUser is null? " + (currentUser == null));
                    ...
                    if (currentUser != null) {
                        ...
                        Executors.newSingleThreadExecutor().execute(() -> {
                            ...
                            android.util.Log.d("UserProfileActivity", "✅ Profile photo saved to local database: " + selectedImageUri);
                            
                            // ✅ ALSO SAVE TO NEON DATABASE (for multi-device sync)
                            saveProfilePictureToNeon(selectedImageUri);
                        });
                    } else {
                        android.util.Log.e("UserProfileActivity", "❌ ERROR: Cannot save profile photo - currentUser is NULL!");
                        ...
                        Executors.newSingleThreadExecutor().execute(() -> {
                            ...
                            if (user != null) {
                                ...
                                currentUser = user;
                                android.util.Log.d("UserProfileActivity", "✅ Profile photo saved after loading user");
                                
                                // ✅ ALSO SAVE TO NEON DATABASE (for multi-device sync)
                                saveProfilePictureToNeon(selectedImageUri);
                            } else {
                                android.util.Log.e("UserProfileActivity", "❌ Still cannot find user with ID: " + userId);
                            }
                        });
                    }
    ...
    private void loadUserData() {
        int userId = preferencesManager.getUserId();
        android.util.Log.d("UserProfileActivity", "=== LOADING USER DATA ===");
        android.util.Log.d("UserProfileActivity", "UserID from PreferencesManager: " + userId);
        ...
        Executors.newSingleThreadExecutor().execute(() -> {
            ...
            for (User u : allUsers) {
                android.util.Log.d("UserProfileActivity", "  User: ID=" + u.getId() + ", Username=" + u.getUsername() + ", FirstName=" + u.getFirstName());
            }
            
            currentUser = database.userDao().getUserById(userId);
            
            android.util.Log.d("UserProfileActivity", "User loaded: " + (currentUser != null ? "YES" : "NULL"));
            if (currentUser != null) {
                android.util.Log.d("UserProfileActivity", "FirstName: " + currentUser.getFirstName());
                android.util.Log.d("UserProfileActivity", "LastName: " + currentUser.getLastName());
                android.util.Log.d("UserProfileActivity", "Username: " + currentUser.getUsername());
            } else {
                android.util.Log.e("UserProfileActivity", "ERROR: User with ID " + userId + " NOT FOUND!");
            }
            ...
            .enqueue(new retrofit2.Callback<java.util.Map<String, Object>>() {
                ...
                public void onResponse(retrofit2.Call<java.util.Map<String, Object>> call,
                    ...
                    if (response.isSuccessful()) {
                        android.util.Log.d("UserProfileActivity", "✅ Profile picture saved to Neon database");
                    } else {
                        android.util.Log.e("UserProfileActivity", "❌ Failed to save to Neon: " + response.code());
                    }
                }
                
                @Override
                public void onFailure(retrofit2.Call<java.util.Map<String, Object>> call, Throwable t) {
                    android.util.Log.e("UserProfileActivity", "❌ Network error saving to Neon: " + t.getMessage());
                }
            });
    }
    
    private void loadProfilePicture() {
        android.util.Log.d("UserProfileActivity", "=== LOADING PROFILE PICTURE ===");
        ...
        Executors.newSingleThreadExecutor().execute(() -> {
            ...
            if (user != null && user.getProfilePhotoUri() != null && !user.getProfilePhotoUri().isEmpty()) {
                ...
                android.util.Log.d("UserProfileActivity", "✅ Loaded URI from database: " + profileImageUri);
            } else {
                android.util.Log.d("UserProfileActivity", "No profile picture in database");
            }
            ...
            runOnUiThread(() -> {
                if (finalProfileImageUri != null && !finalProfileImageUri.isEmpty()) {
                    try {
                        ...
                        android.util.Log.d("UserProfileActivity", "✅ Profile picture loaded");
                    } catch (Exception e) {
                        android.util.Log.e("UserProfileActivity", "❌ Error loading: " + e.getMessage());
                        ivProfilePicture.setImageResource(R.drawable.ic_person);
                    }
                } else {
                    android.util.Log.d("UserProfileActivity", "Using default icon");
                    ivProfilePicture.setImageResource(R.drawable.ic_person);
                }
            });
        });
            ...
            try {
                ...
                for (com.example.blottermanagementsystem.data.entity.Person person : persons) {
                    ...
                    if (personFullName.trim().equals(userFullName.trim())) {
                        ...
                        android.util.Log.d("UserProfileActivity", "✅ Person record deleted: " + personFullName);
                        break;
                    }
                }
                ...
                runOnUiThread(() -> {
                    ...
                    android.util.Log.e("UserProfileActivity", "Error deleting account", e);
                });
            }
        });
    }
